...
const setRenderer = () => {
  ...
  const rendererWidth = width * pixelRatio;
  const rendererHeight = height * pixelRatio;

  const renderer = new THREE.WebGLRenderer({ canvas: tempCanvas, alpha: false, antialias: true });
  renderer.setPixelRatio(pixelRatio);
  renderer.setScissorTest(true);
  renderer.setSize(rendererWidth, rendererHeight); // ! toto
  ...
  renderer.setAnimationLoop(() => {
    ...
    // renderer.setViewport(0, 0, rendererWidth / pixelRatio, rendererHeight / pixelRatio); // THIS WORKS
    // renderer.setViewport(0, 0, width, height);
    // renderer.setViewport(0, 0, rendererWidth, rendererHeight);
    // renderer.setViewport(0, 0, sizes.width, sizes.height);
    renderer.setScissor(0, 0, rendererWidth, rendererHeight);
    renderer.setViewport(0, 0, rendererWidth / pixelRatio, rendererHeight / pixelRatio);
    ...
    ctx.drawImage(
      rendererCanvas,
      0,
      rendererCanvas.height - height * pixelRatio,
      rendererWidth,
      rendererHeight,
      0,
      0,
